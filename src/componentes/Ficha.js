...
import { faCheckCircle, faExclamationTriangle, faSmileWink } from '@fortawesome/free-solid-svg-icons';
...
const Ficha = ({  }) => {
  ...
  const [departamentoTrab, setDepartamentoTrabNew] = useState({ guid: '', nombre: '' });
  const [provinciaTrab, setProvinciaTrabNew] = useState({ guid: '', nombre: '' });
  ...
  const [cargaExitosaSubModalidad, setcargaExitosaSubModalidad] = useState(true);
  ...
  const handleVistas = (id, nombre, url, visible) => {
    setvistaCargaDocumento({ id: id, nombre: nombre, url: url, visible: visible });
